...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is not None and bcrypt.check_password_hash(
            ...
            tok_verified = pyotp.TOTP(user.otp_secret).verify(form.token.data)
            if not tok_verified:
...
def qr_code():
    ...
    uri = pyotp.totp.TOTP(user.otp_secret).provisioning_uri(name=user.username, issuer_name='ESTRELLA-2FA')
